...
template<typename Kernel_traits, bool Is_even_N, bool Is_even_K>
__global__ void flash_fwd_seqk_parallel_kernel(Flash_fwd_params params) {
    flash::compute_attn_seqk_parallel<Kernel_traits, Is_even_N, Is_even_K, true>(params);
}

template<typename Kernel_traits, int Log_max_splits, bool Is_even_K>
__global__ void flash_fwd_combine_seqk_parallel_kernel(Flash_fwd_params params) {
    ...
    BOOL_SWITCH(is_even_N, IsEvenNConst, [&] {
        BOOL_SWITCH(is_even_K, IsEvenKConst, [&] {
            BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                ...
                // auto kernel = &flash_fwd_kernel<Kernel_traits, false, Is_causal, IsEvenNConst, true, false>;
...
void run_flash_seqk_parallel_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    constexpr size_t smem_size = Kernel_traits::kSmemSize;
    const int num_m_block = (params.seqlen_q + Kernel_traits::kBlockM - 1) / Kernel_traits::kBlockM;
    // const int num_n_block = (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN / 4;
    // const int num_n_block = 10;
    dim3 grid(num_m_block, params.num_splits, params.b * params.h);
    // We also use is_even_N to set Unpadded in the BlockInfo constructor, so we need to check
    // for cu_seqlens_q as well.
    // const bool is_even_N = params.cu_seqlens_q == nullptr && params.cu_seqlens_k == nullptr && params.seqlen_k % Kernel_traits::kBlockN == 0;
    const bool is_even_N = false;
    const bool is_even_K = params.d == Kernel_traits::kHeadDim;
    BOOL_SWITCH(is_even_N, IsEvenNConst, [&] {
        BOOL_SWITCH(is_even_K, IsEvenKConst, [&] {
            // auto kernel = &flash_fwd_seqk_parallel_kernel<Kernel_traits, IsEvenNConst, IsEvenKConst>;
            auto kernel = &flash_fwd_seqk_parallel_kernel<Kernel_traits, false, IsEvenKConst>;
            if constexpr (smem_size >= 48 * 1024) {
                C10_CUDA_CHECK(cudaFuncSetAttribute(
                    kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
            }
            ...
            if (params.num_splits <= 2) {
                flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 1, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            } else if (params.num_splits <= 4) {
                flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 2, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            } else if (params.num_splits <= 8) {
                flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 3, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            } else if (params.num_splits <= 16) {
                flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 4, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            } else if (params.num_splits <= 32) {
                flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 5, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            // } else if (params.num_splits <= 64) {
            //     flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 6, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            // } else if (params.num_splits <= 128) {
            //     flash_fwd_combine_seqk_parallel_kernel<Kernel_traits, 7, IsEvenKConst><<<grid_combine, Kernel_traits::kNThreads, 0, stream>>>(params);
            }
            C10_CUDA_KERNEL_LAUNCH_CHECK();
        });
    });
...
void run_mha_fwd_hdim64(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    BOOL_SWITCH(params.p_dropout < 1.f, Is_dropout, [&] {
        BOOL_SWITCH(params.is_causal, Is_causal, [&] {
            ...
            // }
            // run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 128, 128, 4, false, false, T>, Is_dropout, Is_causal>(params, stream);
            // run_flash_fwd<Flash_fwd_kernel_traits<Headdim, 64, 256, 4, false, false, T>, Is_dropout, Is_causal>(params, stream);
        });
    });
    run_flash_seqk_parallel_fwd<Flash_fwd_kernel_traits<Headdim, 64, 256, 4, false, false, T>>(params, stream);
}
...
void run_mha_fwd_hdim128(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    //     });
    // });
    run_flash_seqk_parallel_fwd<Flash_fwd_kernel_traits<Headdim, 64, 128, 4, false, false, T>>(params, stream);
}
