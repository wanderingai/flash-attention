...
template<typename Kernel_traits, bool Is_even_N, bool Is_even_K, bool Seq_parallel, typename Params>
inline __device__ void compute_attn_1rowblock_seqk_parallel(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    const BlockInfo</*Varlen=*/!Is_even_N> binfo(params, bidb);
    // TODO: We might need to write sth to the LSE and Oaccum, so that when we combine the outputs
    // we don't get wrong results. However, right now we set the num_splits such that we'll never
    // have a block that doesn't do any work.
    if (m_block * kBlockM >= binfo.actual_seqlen_q || binfo.actual_seqlen_k == 0) return;

    const int n_blocks_per_split = ((params.seqlen_k + kBlockN - 1) / kBlockN + num_n_splits - 1) / num_n_splits;

    int n_block_max = std::min(int(cute::ceil_div(binfo.actual_seqlen_k, kBlockN)), (n_split_idx + 1) * n_blocks_per_split);
    const int n_block_min = n_split_idx * n_blocks_per_split;
    if (n_block_min >= n_block_max) return;  // TODO: do we need to write 0 to out here?
    ...
    const index_t row_offset_o_accum = (((n_split_idx * params.b + bidb) * params.h + bidh) * params.seqlen_q
    ...
    copy(smem_tiled_copy_Oaccum, taccOrOaccum, taccOsOaccum);

    // const index_t row_offset_o = binfo.q_offset(params.o_batch_stride, params.o_row_stride, bidb)
    //     + m_block * kBlockM * params.o_row_stride + bidh * params.o_head_stride;
    const index_t row_offset_lse = ((n_split_idx * params.b + bidb) * params.h + bidh) * params.seqlen_q + m_block * kBlockM;
    // Tensor gO = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.o_ptr) + row_offset_o),
    //                         Shape<Int<kBlockM>, Int<kHeadDim>>{},
    //                         make_stride(params.o_row_stride, _1{}));
    Tensor gLSE = make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.softmax_lse_accum_ptr) + row_offset_lse),
                              Shape<Int<kBlockM>>{}, Stride<_1>{});


    Tensor gOaccum = make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.o_accum_ptr) + row_offset_o_accum),
                                 Shape<Int<kBlockM>, Int<kHeadDim>>{},
    ...
    // // Reshape acc_o from (4, 1, 2) to (4, 2, 1) to write to gdQaccum
    // Tensor acc_o_reshaped = make_tensor(acc_o.data(),
    //                                     make_layout(get<0>(acc_o.layout()),
    //                                                 get<2>(acc_o.layout()),
    //                                                 get<1>(acc_o.layout())));
    // if (cute::thread0()) { print(acc_o_reshaped); print(tOgOaccum); }

    __syncthreads();

    Tensor tOrOaccum = make_tensor<ElementAccum>(shape(tOgOaccum));
    copy(gmem_tiled_copy_Oaccum, tOsOaccum, tOrOaccum);
...
template<typename Kernel_traits, bool Is_even_N, bool Is_even_K, bool Seq_parallel, typename Params>
inline __device__ void compute_attn_seqk_parallel(const Params &params) {
    const int m_block = blockIdx.x;
    // The block index for the batch.
    ...
    flash::compute_attn_1rowblock_seqk_parallel<Kernel_traits, Is_even_N, Is_even_K, Seq_parallel>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    Tensor gLSEaccum = make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.softmax_lse_accum_ptr) + row_offset_lse),
    ...
    const index_t row_offset_o_accum = bidx * kBlockM * params.d_rounded;
    Tensor gOaccum = make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.o_accum_ptr) + row_offset_o_accum),
                                 Shape<Int<kBlockM>, Int<kHeadDim>>{},
                                 Stride<Int<kHeadDim>, _1>{});
    typename Kernel_traits::GmemTiledCopyOaccum gmem_tiled_copy_Oaccum;
